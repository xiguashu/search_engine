...
static void stemfile(FILE * f)
{
	while (TRUE)
	{
		...
		if (LETTER(ch))
		{
			...
			fprintf(out, "%s", s);/*改为输到一个文件中*/
		}
		else {
			fputc(ch, out);
			fputc(0x0D, out);/*为了代替原算法的putchar，在windows下再输一个回车,原算法是一个putchar（ch）输到屏幕*/
		}
	}
}
...
int main(int argc, char** argv) {
	...
	for (int j = 0;j < size;j++)//对每篇文档提取词干. 
	{
		string name, old; 
		char*cname, *cold;
		old = files[j].c_str();
		cold = &old[0];//为了使用c语言的文件函数 
		f = fopen(cold, "r");
		if (f == NULL) { printf("File not found\n"); exit(12); }
		name = "G:\\0-programming\\Little Search Engine\\0-3-stemmedFile\\stemmed-" + old.substr(sizeof("G:\\0-programming\\Little Search Engine\\0-2-antStopwordFile\\antiSW"));
		cname = &name[0];
		out = fopen(cname, "w");
